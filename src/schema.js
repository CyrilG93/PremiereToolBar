(function (root, factory) {
  ...
  const ACTION_IDS = ["copyClipEffects", "extendClipInToPlayhead", "extendClipOutToPlayhead", "invertTimelineSelection", "moveVideoClipDown", "moveVideoClipUp", "pasteClipEffects", "randomizeTimelineClipOrder", "removeClipEffects", "reverseTimelineClipOrder", "reverseTowerVideoClips", "setClipLabel", "staircaseVideoClipsDown", "staircaseVideoClipsUp", "towerOffsetVideoClips", "towerVideoClips"];
  ...
  function parseImportJson(json) {
    ...
    if (Array.isArray(parsed && parsed.collections) && Array.isArray(parsed && parsed.buttons)) {
      const normalized = normalizeConfig({
        ...
        // Preserve complete-pack bar layout, sizing, and assignments during import normalization.
  ...
  function importJson(config, json, options) {
    ...
    if (imported.bars && imported.bars.length) {
      normalized.bars = normalized.bars.map((bar) => {
        ...
        return mappedCollectionId ? createBar(Object.assign({}, bar, importedBar, { collectionId: mappedCollectionId }), BAR_IDS.indexOf(bar.id), normalized.collections[0].id) : bar;
      });
    }
