...
Tool Bar is an Adobe Premiere Pro UXP extension that adds compact dockable shortcut for common editing actions.

Create buttons for native effects, video transitions, captured presets, timeline actions, and utility actions, then place them in one or more toolbar collections.
...
- `Actions`: copy/paste/remove effects, set a clip label, move selected video clips to the first free track up or down, create an ascending/descending staircase, build a tower, set selected clip In/Out points at the playhead, or invert the timeline selection. Set Clip Label applies the selected Project panel label to the source media, so every timeline occurrence of the same media follows; custom colors still apply, although their personalized names cannot be read by Premiere UXP. The In/Out actions work with selected video and audio clips across several tracks, never clone clips, and can extend or shorten a clip while preserving a positive duration. Invert Timeline Selection selects every regular video and audio clip that was not selected, and clears the previous selection. A tower places video or audio clips on consecutive tracks and aligns their timeline In points with the first clip. Audio linked to a selected video follows when it has the same source media and timeline range.
- `Tools`: open Settings.
