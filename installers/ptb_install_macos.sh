...
# // Read the plugin version from manifest.json, or infer it from the release CCX when the full source is absent.
if [ -f "${PTB_MANIFEST}" ]; then
  PTB_VERSION="$(/usr/bin/awk -F '"' '/"version"[[:space:]]*:/ { print $4; exit }' "${PTB_MANIFEST}")"
else
  PTB_RELEASE_CANDIDATE="$(/usr/bin/find "${PTB_ROOT}" -maxdepth 1 -type f -name "ToolBar-*.ccx" | /usr/bin/sort | /usr/bin/tail -n 1)"
  PTB_RELEASE_FILE="$(basename "${PTB_RELEASE_CANDIDATE:-}")"
  PTB_VERSION="${PTB_RELEASE_FILE#ToolBar-}"
  PTB_VERSION="${PTB_VERSION%.ccx}"
fi
...
# // Preserve existing user buttons when the backup helper is present in a source checkout.
if [ -f "${PTB_ROOT}/scripts/ptb-backup-macos.sh" ]; then
...
# // Use the release CCX when it is already included beside the README.
PTB_BUILD_DIR="${PTB_ROOT}/.ptb-installer-build"
PTB_RELEASE_CCX="${PTB_ROOT}/ToolBar-${PTB_VERSION}.ccx"
PTB_CCX="${PTB_RELEASE_CCX}"
if [ -f "${PTB_RELEASE_CCX}" ]; then
  echo "Using included release package: ${PTB_CCX}"
else
  # // Stage only the files required by the UXP plugin runtime when running from a source checkout.
